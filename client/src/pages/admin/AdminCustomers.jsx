      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between border-b border-slate-900 pb-5">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">Review accounts registered in the e-commerce database.</p>
          </div>
          <button onClick={fetchCustomers} className="text-slate-500 hover:text-slate-350">
            <RefreshCw size={18} className={loading ? 'animate-spin' : ''} />
          </button>
        </div>

        {error && (
          <div className="bg-rose-500/10 border border-rose-500/20 text-rose-400 p-3.5 rounded-xl text-xs">
            {error}
          </div>
        )}
