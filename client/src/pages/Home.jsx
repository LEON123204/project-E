...
const Home = () => {
  ...
  const [categoriesRef, categoriesVisible] = useScrollReveal({ threshold: 0.1, triggerOnce: true });
  const [statsRef, statsVisible] = useScrollReveal({ threshold: 0.15, triggerOnce: true });
  const [productsRef, productsVisible] = useScrollReveal({ threshold: 0.05, triggerOnce: true });
  ...
  const categories = [
    { name: 'Electronics', slug: 'electronics', desc: 'Noise cancelling gear, mechanical keyboards, smart watches', img: 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=600&auto=format&fit=crop&q=60' },
    { name: 'Fashion & Apparel', slug: 'fashion-apparel', desc: 'Premium leather, denim, backpacks, and knitwear', img: 'https://images.unsplash.com/photo-1551028719-00167b16eac5?w=600&auto=format&fit=crop&q=60' },
    { name: 'Home & Living', slug: 'home-living', desc: 'Diffusers, iron skillets, linens, and desk chairs', img: 'https://images.unsplash.com/photo-1608571423902-eed4a5ad8108?w=600&auto=format&fit=crop&q=60' },
    { name: 'Fitness & Outdoors', slug: 'fitness-outdoors', desc: 'Hydration flasks, yoga mats, kettlebells, and tents', img: 'https://images.unsplash.com/photo-1526506118085-60ce8714f8c5?w=600&auto=format&fit=crop&q=60' },
    { name: 'Books & Stationery', slug: 'books-stationery', desc: 'Dotted journals, fineliners, organizers, and fountain pens', img: 'https://images.unsplash.com/photo-1531346878377-a5be20888e57?w=600&auto=format&fit=crop&q=60' },
    { name: 'Accessories', slug: 'accessories', desc: 'Premium wallets, polarized sunglasses, and silk ties', img: '/uploads/leather_wallet.png' },
    { name: 'Skincare', slug: 'skincare', desc: 'Hyaluronic serums, organic face wash, and body creams', img: '/uploads/hyaluronic_serum.png' },
    { name: 'Car & Bike Accessories', slug: 'car-bike-accessories', desc: 'Phone mounts, steering covers, U-locks, and bike lights', img: '/uploads/car_phone_mount.png' }
  ];
  ...
  return (
    <div className="bg-slate-950 text-slate-100 min-h-screen pb-16 overflow-x-hidden">
      ...
      {/* Featured Categories */}
      <section ref={categoriesRef} id="categories" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 scroll-mt-16">
        <div className={`transition-all duration-1000 ease-out transform ${categoriesVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
          <div className="text-center md:text-left mb-10 flex flex-col md:flex-row items-end justify-between gap-4">
            <div>
              <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-100">Shop by Category</h2>
              <p className="text-slate-500 mt-2">Find exactly what you need to upgrade your style and surroundings.</p>
            </div>
            <Link to="/shop" className="text-indigo-400 hover:text-indigo-300 font-semibold flex items-center gap-1 text-sm transition-smooth group">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {categories.map((cat, index) => (
              <Link
                ...
                style={{
                  transitionDelay: categoriesVisible ? `${index * 75}ms` : '0ms'
                }}
              ...
              >
                ...
                <div className="z-20 space-y-2">
                  <h3 className="text-lg font-bold text-slate-100 group-hover:text-indigo-300 transition-smooth">{cat.name}</h3>
                  <p className="text-xs text-slate-300 line-clamp-2 leading-relaxed">{cat.desc}</p>
                </div>
              </Link>
            ))}
          </div>
      ...
      {/* Stats Section */}
      <section ref={statsRef} className="bg-slate-900/20 border-y border-slate-900/60 py-16 my-8">
        <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 transition-all duration-1000 ease-out transform ${statsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            <div className="space-y-2">
              <p className="text-4xl sm:text-5xl font-black tracking-tight text-indigo-400">
                <AnimatedCounter target={10000} suffix="+" />
              </p>
              <p className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-500">Happy Customers</p>
            </div>
            <div className="space-y-2">
              ...
              <p className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-500">Premium Products</p>
            </div>
            <div className="space-y-2">
              <p className="text-4xl sm:text-5xl font-black tracking-tight text-pink-400">
                <AnimatedCounter target={25} suffix="+" />
              </p>
              <p className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-500">Stores Nationwide</p>
            </div>
            <div className="space-y-2">
              <p className="text-4xl sm:text-5xl font-black tracking-tight text-emerald-400">
                24/7
              </p>
              <p className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-500">Expert Support</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Trending Products */}
      <section ref={productsRef} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className={`transition-all duration-1000 ease-out transform ${productsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
          <div className="text-center md:text-left mb-10">
            <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-100 font-sans">New Arrivals</h2>
            <p className="text-slate-500 mt-2">Explore the freshest additions to our inventory, hot off the shelves.</p>
          </div>
